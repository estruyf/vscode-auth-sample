import { authentication, AuthenticationProvider, AuthenticationProviderAuthenticationSessionsChangeEvent, AuthenticationSession, Disposable, env, EventEmitter, ExtensionContext, ProgressLocation, UIKind, Uri, UriHandler, window } from "vscode";
...
const REDIRECT_URL = `https://vscode-redirect-9ajo.vercel.app`;
...
interface Auth0AuthenticationSession extends AuthenticationSession {
  refreshToken: string;
}
...
export class Auth0AuthenticationProvider implements AuthenticationProvider, Disposable {
   ...
   * @returns 
   */
  public async getSessions(scopes?: string[]): Promise<readonly Auth0AuthenticationSession[]> {
    try {
      ...
      const sessions = JSON.parse(allSessions) as Auth0AuthenticationSession[];
      if (sessions) {
        if (allScopes && scopes) {
   ...
   * @returns 
   */
  public async createSession(scopes: string[]): Promise<Auth0AuthenticationSession> {
    try {
      ...
      const session: Auth0AuthenticationSession = {
        id: uuid(),
